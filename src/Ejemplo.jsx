...
const SUPABASE_ANON_KEY = 'tu-anon-key-aqui'; // Reemplazar con tu API key
...
const Ejemplo = () => {
  ...
  const fetchSessions = async () => {
    ...
    setError(null);
    try {
      const response = await fetch(`${SUPABASE_URL}/rest/v1/sessions?select=*&order=date.desc`, {
        headers: {
      ...
      if (!response.ok) {
        throw new Error('Error al cargar sesiones desde Supabase');
      }
...
4. Recomendaciones personalizadas`;
  ...
  const handleSaveSession = async () => {
    if (currentSession) {
      setLoading(true);
      try {
        const savedSession = await saveSessionToSupabase(currentSession);
        ...
        setError('No se pudo guardar la sesión. Por favor, intenta de nuevo.');
      } finally {
        setLoading(false);
      }
    }
  ...
  const handleAIConsult = async () => {
    ...
    try {
      // Consultar ambas IAs en paralelo
      const [deepseekResponse, copilotResponse] = await Promise.all([
        consultDeepSeek(currentSession),
        consultCopilot(currentSession)
      ]);

      setAiResponses({
        deepseek: {
          name: 'DeepSeek (Análisis Clínico)',
          diagnosis: deepseekResponse,
          recommendation: 'Recomendaciones incluidas en el análisis'
        },
        copilot: {
          name: 'Copilot (Análisis Predictivo)',
          diagnosis: copilotResponse,
          recommendation: 'Recomendaciones incluidas en el análisis'
        }
      });
    } catch (err) {
      setError('Error al consultar las IAs. Mostrando análisis de respaldo.');
      // Fallback con análisis básico
      setAiResponses({
        deepseek: {
          name: 'DeepSeek (Análisis Clínico)',
          diagnosis: `Basándome en los KPIs: frecuencia ${currentSession.kpis.frequency} episodios/mes, intensidad ${currentSession.kpis.intensity}/10, y duración ${currentSession.kpis.duration}h, sugiero:\n\n- Migraña episódica con patrón ${currentSession.kpis.intensity > 6 ? 'moderado-severo' : 'leve-moderado'}\n- Considerar ajuste de dosis preventiva\n- Evaluar terapia combinada si frecuencia >4/mes\n- Seguimiento de desencadenantes: ${currentSession.kpis.triggers}`,
          recommendation: 'Continuar tratamiento actual con revisión en 2 semanas'
        },
        copilot: {
          name: 'Copilot (Análisis Predictivo)',
          diagnosis: `Análisis de patrones detectados:\n\n- Tendencia ${currentSession.kpis.frequency < 3 ? 'positiva' : 'a monitorear'} en frecuencia\n- Intensidad del dolor requiere ${currentSession.kpis.intensity > 7 ? 'intervención inmediata' : 'seguimiento estándar'}\n- Factores desencadenantes identificados: ${currentSession.kpis.triggers}\n- Probabilidad de mejora con tratamiento actual: ${currentSession.kpis.intensity < 6 ? 'alta (75%)' : 'moderada (55%)'}`,
          recommendation: 'Implementar diario de desencadenantes y considerar técnicas de manejo de estrés'
        }
      });
    } finally {
      setLoadingAI(false);
